...
async def summarize_with_gemini(transcription: str, duration_mins: float, listing_data: str = "") -> str:
    ...
    prompt = (
        ...
        f"PROPERTY SPECS BLOCK (at the very top, if listing data is available):\n"
        f"  Extract and present each of the following on its own line if found in the listing data:\n"
        f"  • Bedrooms / Bathrooms\n"
        f"  • Size (sq ft / range feet²)\n"
        f"  • Lot Size\n"
        f"  • Basement (type, finished/unfinished, walk-out, suite, etc.)\n"
        f"  • Days on Market\n"
        f"  • Property Tax (annual, with year)\n"
        f"  • Asking Price\n"
        f"  • Parking / Garage\n"
        f"  • Any other notable specs (heating, AC, pool, etc.)\n\n"
        f"BROKER DESCRIPTION:\n"
        f"  After the specs, include the key points from the broker/agent marketing description\n"
        f"  as concise bullet points (upgrades, special features, suite potential, etc.).\n\n"
        f"WALKTHROUGH SUMMARY:\n"
        f"  Summarize what the agent said during the recording in the same language(s) as the speaker.\n"
        f"  - Farsi parts → Persian script (فارسی), English parts → English\n"
        f"  - Use clear sections/bullet points\n"
        f"  - Cover: rooms visited, condition notes, highlights, concerns mentioned\n"
